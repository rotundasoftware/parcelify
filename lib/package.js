...
Package.prototype.createAsset = function( thisAssetSrcPath, assetType ) {
	...
	log.info( '', assetType + ' asset registered "%s"', thisAssetSrcPath );
...
Package.prototype.writeAssetsToDisk = function( assetTypesToWriteToDisk, outputDirectoryPath, callback ) {
	...
	assetTypesToWriteToDisk = _.intersection( assetTypesToWriteToDisk, Object.keys( _this.assetsByType ) );
	...
	async.each( assetTypesToWriteToDisk, function( thisAssetType, nextAssetType ) {
		async.each( _this.assetsByType[ thisAssetType ], function( thisAsset, nextAsset ) {
			thisAssetDstPath = path.join( outputDirectoryPath, path.relative( _this.path, thisAsset.srcPath ) );
			...
			thisAsset.writeToDisk( thisAssetDstPath, nextAsset );
		}, nextAssetType );
	}, function( err ) {
		if( err ) return callback( err );

		return callback( null, pathsOfWrittenAssets );
	} );
};
...
Package.prototype._createPackageJsonWatcher = function( assetTypes, packageTransform ) {
	...
	this.assetJsonWatcher.on( 'changed', function( srcPath ) {
		log.info( 'watch', 'package.json changed "%s"', srcPath );
		...
		fs.readFile( srcPath, 'utf8', function( err, packageJson ) {
			...
			if( packageTransform ) packageJson = packageTransform( packageJson );
...
Package.prototype._createAssetGlobWatchers = function() {
	...
	_.each( _this.assetGlobsByType, function( globs, thisAssetType ) {
		...
		thisWatcher.on( 'changed', function( srcPath ) {
			try {
				log.info( 'watch', '"%s" changed', srcPath );
		...
		thisWatcher.on( 'added', function( srcPath ) {
			try {
				log.info( 'watch', '"%s" added', srcPath );
		...
		thisWatcher.on( 'deleted', function( srcPath ) {
			try {
				log.info( 'watch', '"%s" deleted', srcPath );
