...
Parcel.prototype.writeBundle = function( assetType, dstPath, callback ) {
	...
	async.series( [ function( nextSeries ) {
		...
		async.eachSeries( srcAssets, function( thisSrcAsset, nextSrcAsset ) {
			if( thisSrcAsset.streamSpent ) thisSrcAsset.createStream();

			thisSrcAsset[ 'stream' ].on( 'end', function() {
				thisSrcAsset.streamSpent = true;
				nextSrcAsset();
			} );

			thisSrcAsset[ 'stream' ].pipe( bundle, { end : false } );
		}, function( err ) {
